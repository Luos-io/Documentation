...
![img alt](/img/blog/7.png)

This post will introduce the basics of PID control and why it comes in handy for robotic applications. Discover here an introduction to PID control with DC motor. You don't need much background to read this, but knowing the basics of motors is always a good idea: my previous posts on this subject are listed here: [Welcome to the jungle](https://www.luos.io/blog/dc-motor-control-welcome-to-the-jungle).
...
Don't hesitate to try our technology by following [our getting started tutorial.](https://docs.luos.io/get-started/get-started) Three steps are needed to become a Luos user!
...
> **_Note:_** _Here, I use the same word — control — that in the post_ **_How to stop being controlled by your DC motor: reverse the roles!_** _(_[_part 1: direction_](https://www.luos.io/blog/how-to-stop-being-controlled-by-your-dc-motor-reverse-the-roles) _and_ [_part 2: speed_](https://www.luos.io/blog/how-to-stop-being-controlled-by-your-dc-motor-reverse-the-roles-2)_). Control is a generic word, and the control I'm talking about today is closer than engineering control, which is more complex than giving a simple speed order to a motor._

Let's first define the **system** as:

*   An actuator (for example a motor) and its control board (e.g. [this board](https://www.pololu.com/product/713))
...
These three terms (components) will be calculated in function of the error given by the sensor. Let's call these components functions to simplify things: e.g., _F\_i(e)_ is the integral function of the error _e_. The original target has reached the set point.
...
*   [PID: the P, as in Proportional](https://www.luos.io/blog/pid-the-p-as-in-proportional)
*   [PID: the I, as in Integral](https://www.luos.io/blog/pid-the-i-as-in-integral)
*   [PID: D, as in Derivative](https://www.luos.io/blog/pid-d-as-in-derivative)
...
We will dig in these [proportional](https://www.luos.io/blog/pid-the-p-as-in-proportional), [integral](https://www.luos.io/blog/pid-the-i-as-in-integral), and [derivative](https://www.luos.io/blog/pid-d-as-in-derivative) components on the following posts. We'll look at the maths behind PID control and how to adjust the three constants to get the desired behavior from our motor.

PID control and many other projects are waiting for you on our blog, so don't hesitate to bookmark our content and [join our Discord community](https://discord.gg/luos)

Still on the same subject, read now the following article on the [PID: the P as in Proportional](https://www.luos.io/blog/pid-the-p-as-in-proportional).
