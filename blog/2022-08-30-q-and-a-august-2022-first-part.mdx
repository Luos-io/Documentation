...
image: /img/blog/q-and-a-session-Luos-august-2022-first-part.jpg
hide_table_of_contents: false
date: 2022-08-30T11:00
---

![What is the difference between Luos and an embedded OS like ROS? Q&A live](/img/blog/q-and-a-session-Luos-august-2022-first-part.jpg)
...
Luos is not an operating system. This technology allows the developers to **encapsulate any embedded system features** into something we call [services](/docs/luos-technology/services). 

A service is a kind of a box with an API on top of it. Luos will allow you to exchange information between the different services available on your entire system using any network. Boxes communicate together. So basically, this is not an operating system: this is a way to separate your developments into small services.
...
The idea is to declare your different features — for example, a motor, a battery, or an algorithm to filter some sensor's values. 
...
Luos is not so complicated for embedded developers. The code side is easy. You have to master something like two or three different functions. The difficult things to [get started](/tutorials/get-started) with Luos are concepts of microservices and how to deal with them in the embedded world.

[Microservices](/2021-01-13-unleash-embedded.mdx) are more focused on the web domain than it is on the regular hardware world, and embedded development doesn't use this kind of methodologies.

Luos combined those two worlds, allowing embedded system developers to think of their product development as a whole **microservice system**. This is the biggest characteristic that is difficult to understand. This is how to use microservice in the embedded world and make it efficient.
...
You can interact from a program on your computer directly with a program embedded directly into electronic boards (such as a sensor, a battery, or a motor). This allows normal software developers to interact with the embedded world easily, and we have a lot of users dealing with it this way. 
...
So, it's easy! We already provided some frameworks, allowing you to deal with Luos using [Python](/docs/tools/pyluos), [ROS](/2022-08-16-ros-controlling-your-distributed-robot-using-micro-ros-and-luos.mdx), etc.

<h2>How do you address the fact that orchestrators in embedded systems are highly dependent on the project in which they are used?</h2>
...
When you develop with Luos, you will be able to have small strike teams working on only one piece of the device, for example the motion sensor of one device, while some other team will work on the motor part of the device.

These two teams don't have to talk together. The idea of Luos is to allow them to develop their services on their side, and then to just gather to plug them together: it will work. This is the main idea.

So this is a big change for big companies, and right now we have some users coming from critical industries: we already have some advanced use cases such as <a href="https://nimbl-bot.com/" rel="external nofollow">Nimbl'bot</a>, a company developing a robot's arm for nuclear plants. They are using these robots directly in the reactor of the nuclear plants. This is a highly [critical usage of Luos](/2022-04-28-distributed-latency.mdx).
...
<h2>About industrial applications: do Luos has a CAN bus driver? Is Luos working with Zephyr, NuttX, and FreeRTOS? Is Luos only giving a wrapper?</h2>
...
You can have motor drivers on a board, it can just run on a really small board without any real-time operating system, and have another board running some complex algorithm using that kind of motor running on RTOS, such as NuttX or [FreeRTOS](/tutorials/freertos/intro).
...
Embedded system developers didn't use to care about a project's API. But we think that today their points of view are evolving toward using this kind of standards, because everyone is using it. It allows them to create developments that will be reusable extremely more easily.
...
We did an experiment using [FreeRTOS](/tutorials/freertos/intro) just a few months ago: it that was easy for us to adapt it to work with FreeRTOS. 

Luos was then ready to multithreading. Although this is something new to us, we plan to develop a lot of support for different kinds of real-time operating systems in the future, so NuttX is on top of the list right now, thanks to Alan. We will work on this kind of subject during the coming months.
...
<a href="/blog/are-microservices-optimized-for-edge-and-embedded-q-and-a" class="pagination-nav__link" style={{ display: 'inline-block' }}>
