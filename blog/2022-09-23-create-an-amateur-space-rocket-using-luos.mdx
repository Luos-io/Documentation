...
- The MOSI is the line where the follower write and the initiator reads.

- The MISO is the line where the initiator writes and the followers.
...
- The difference here is that this service does not reply to a message, so it needs to identify the other services first.
  ...
  - Therefore, this service does a [Luos Detection](/tutorials/your-first-detection). This functionality allows us to have a map of the  and know what and where the other services are. Then, it stores the ID of these services in variables to be able to communicate with them at any moment.
