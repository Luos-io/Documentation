...
image: /assets/images/blog/a-flexible-embedded-bootloader-luos.jpg
...
![A flexible embedded bootloader: updating becomes easier](/assets/images/blog/a-flexible-embedded-bootloader-luos.jpg)
...
Each company develops its own [bootloader](https://www.luos.io/docs/tools/bootloader), the current operation is that to flash an MCU, it is necessary to connect to the card physically, use an IDE to launch the command and update the firmware. The update was more complex in the case of several cards of different brands.

<h2>Part 2: Bootloaders in embedded systems: how does it work?</h2>

In conventional **embedded systems**, the uploading of a firmware on an MCU is done using several programmers (like stlink, ftdi, dfu, etc). This process is PIN-dependent, that is to say the programming of the binary firmware is done using specific PIN entries (JTAG or SWD) of each board.
...
The main disadvantage of this method is that each board that need to be flashed, should have a **physical access** to a computer. Conventional bootloaders are designed to accept commands directly by a computer. Imagine a distributed robot, where not all the boards are accessible be physically connected to it.

<h4>How could we update their firmware?</h4>

Also, it requires the system to be booted from a specific bootloader image. This can be problematic if the bootloader image is corrupted, or if the system needs to be booted from a different image (how can I update bootloader firmware?)
...
Imagine that you need to <a href="https://www.youtube.com/shorts/NrDbMe79TNA" rel="external nofollow">flash 30 motors of a robot</a>, each time you do a minor modification. You cannot wait several minutes to do this, you need something speeder.

At that point the question was: Do I need the quicker upload possible without any security control, or is it too risky to lose all the effort that is done by not checking the accuracy of the firmware flashed to my boards? The best answer is to find the compromise that combines both.

Finally, the same concern that a user can have when they do not have physical access to their boards, may arrive the moment that we want to update the **bootloader** itself. This bootloader is a part of an **open source project** that constantly evolves, so it would be nice to keep it updated. Can we do it remotely?

So for the questions above, we targeted the four main needs that Luos should meet by developing this revolutionary bootloader which are **Agility**, **Rapidity**, **Security** and **Maintainability**.
...
Whichever your system configuration, through robust network and thanks to a [gate](https://www.luos.io/docs/tools/gate) that communicate between your computer and your embedded system , you can update a board even if it is not directly connected to your computer. 

The **gate** is a specific tool, that can be hosted at any board, and in combination with a **pipe** that transfers the data from a computer to the embedded network and the opposite, it is responsible to convert this data from the format received by the computer (ex. json) to the embedded communication protocol (ex. the Luos communication protocol called [Robus](https://www.luos.io/docs/luos-technology/node/luos-hal#robus-hal)) and the opposite.
...
<h4>Update all your boards whatever the brand</h4>

One of the main constraints when upgrading a system beyond the quality of the bootloader developed, is its compatibility with different systems. Each system has its own **bootloader**. The developed bootloader is available on our <a href="https://github.com/Luos-io/luos_engine" rel="external nofollow">Github</a>, allows using the gate and the [HAL](https://www.luos.io/docs/luos-technology/node/luos-hal) of the board, to **update any brand of card** without having the need to develop a specific bootloader.

Typically, using Luos bootloader you will be able to update an **Arduino**, an **ST**, a **RaspberryPi** board and many others.

But how this bootloader works in a more technical point of view?
...
**Bootloader**: The bootloader is placed (by default) in the beginning of the Flash memory. In each board, when a bootloader exists, we keep a byte in the shared memory, to stock a value that shows if the board is in mode bootloader, or in mode application. Each time the firmware of the board is launched, depending on this value, the execution starts at the beginning of the bootloader, or the application. When we want to flash a firmware, the board gets to bootloader mode, and then the bootloader is responsible for receiving the computer’s commands, regarding the erasing of flash, the allocation of a specific place of memory to stock the firmware, the data chunks, the crc calculation, etc.

**Gate and Pipe**: The data sent from the computer are received by a Luos service called Pipe, and the Pipe streams them directly to the Gate. The Gate converts them to the format of the embedded communication protocol and distributes them to the bootloaders. It does exactly the same process with the opposite direction. By default, the messages coming from a computer are in json format and the embedded communication protocol that is used, is Robus, the custom protocol proposed by Luos. 

**Computer**: The computer software included in Luos Python library called [Pyluos](https://www.luos.io/docs/integrations/pyluos), is responsible to initiate and guide all the procedure. Firstly, it commands the boards that we want to upload to be set in Bootloader mode, and erase the necessary space in flash memory, and then it sends the chunks of the firmware we want to uplaod. Finally it is responsible for checking if the process is completed with success.

<h3>Rapidity</h3>
<h4>Gain time, cost of development and after-sales interventions</h4>
...
Luos bootloader is built-in with Luos engine, it is hardware independent, so you do not need to redevelop one each time you use a new board.

Since you are updating a set of cards through the same means, you will avoid numerous manipulations to connect physically each of your cards. In fact, through the [Luos network](https://www.luos.io/docs/luos-technology/node/topology) and especially through the Gate, updating the cards is much easier. Luos will deliver the right firmware to the right card, no matter which is their physical location on your network. Time saving will also be present in a context of industrialization.
...
A real question, when developing a tool like a bootloader is at which point, it is acceptable to sacrifice the safe execution, for achieving a better performance. It is clear that when you accelerate the time of a process, it is possible to provoke mistakes, and to lose the credibility of your system. For this reason, apart from the protection methods included in Luos comminunication protocol which are **Messages Acknowledgement, Message retry and CRC calculation,** the development of the new features of the bootloader was **enforced with protection mechanisms**, that can provide information regarding the upload results, and when, where and why a fail has arrived.

- **Node readiness:** At the beginning of the computer’s commands we always check if an MCU is ready and in a state that allows it to be uploaded.
- **Erase flash testing:** Before sending the firmware to upload, the computer demands the mcu to erase the space of flash memory that will used to stock the firmware. The bootloader should give a response of the result, if the clean is done, if there is enough space in memory etc.
- **Response testing:** After each binary chunk sent to the board, the bootloader should send back a response that this chunk is valid and well stocked to the memory.
...
Almost all the features mentioned above are included in the Luos engine v2.7.0., while the bootloader upgrade method is included in v2.8.0. However in the embedded tools like a bootloader there is always a margin of amelioration. So, we have already thought about the next steps in order to enforce it with more functionalities and also robustify the existing ones.

<h3>Enforce security: Backup bootloader</h3>

It is previously analyzed the case where an application, for some reason is not successfully uploaded, then the security mechanisms inform the user for the problem and the bootloader is ready to try again. 

Nevertheless, there is a case that we have a failed to update the bootloader, so the data in the position of flash memory corresponding to the main bootloader can be corrupted. In that case the board should be flashed manually, which can cause problems if we do not have access to the board.

A solution could be to stock a second backup bootloader in the a place of the memory that is not used (for example in an other bank etc.), and in case of a corrupted bootloader, the system could jump directly to the backup bootloader, so that we do not lose the functionality of the board.
...
An other useful tool that we are considering integrating to the Luos bootloader is a mechanism to simply swap from the bootloader to the application and the opposite, by sending python commands, using **Pyluos library**. In that way, we could easily get in bootloader mode, even when we are not trying to flash the board, and to the application when we need to relaunch the application of the system, by using only one and simple command.

![Bootloader: How does it work](/assets/images/blog/bootloader-how-does-it-work-luos.svg)

Finally, there is always other modifications to do, to ameliorate all the aspects of the bootloader (speed, maintainance etc.) that will always be a priority for the bootloader development.

These improvements can come from your ideas so feel free to send us some advise on our <a href="https://discord.gg/luos" rel="external nofollow">Discord community</a>.
