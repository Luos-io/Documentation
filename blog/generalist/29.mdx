...
With our first try to connect embedded systems with a Javascript app, we based our development on the [Web Serial API](https://developer.mozilla.org/en-US/docs/Web/API/Web_Serial_API), a “living document” described by the W3C Incubator Community Group (WICG).
...
On the other side, we developed an application service called [Gate](https://docs.luos.io/docs/next/tools/gate), which was intended to convert the Luos “generic software representation” into a JSON format.
...
To do that, we created a tool called [Sniffer](https://docs.luos.io/docs/next/tools/monitoring#sniffer). The Sniffer tool came with a new communication protocol closer to the embedded one but also with drawbacks, and one of them was the design itself; a Sniffer can’t act on the embedded system!
...
![(1) Illustration of the exchange of data between Luos and Luos Engine](https://uploads-ssl.webflow.com/602cf5c87ad04ea98eaa99da/6259793dbddc4081d82ce80e_Illustration_of_the_exchange_of_data_between_Luos_and_Luos_Engine.png)
...
Now that we have set up this new way to communicate with the embedded world, the next step will be to give our users all the features implemented all over the years in [Luos engine](https://github.com/Luos-io/luos_engine) from a web perspective.

The first feature we decided to implement is the network topology. This feature is a graphical representation of the Luos [routing table](https://docs.luos.io/docs/luos-technology/services/routing-table).
...
![Illustration of the routing table data](https://uploads-ssl.webflow.com/602cf5c87ad04ea98eaa99da/6259797c32e8e75fd107891a_Illustration_of_the_routing_table_data.png)

Illustration of the routing table data

![](https://uploads-ssl.webflow.com/602cf5c87ad04ea98eaa99da/62597995554c18dbd4c22015_Services_data.png)

Service’s data

We finally gathered all those data in a graph using the [D3JS](https://observablehq.com/@d3/gallery) library:

![Network Display of Luos web app](https://uploads-ssl.webflow.com/602cf5c87ad04ea98eaa99da/625979ba694083b48af2a748_Network_Display_Luos.png)

Network Display of Luos web app

<h2> What’s next? </h2>

The communication between web and embedded will continue to evolve. Take a look at [this article](/blog/how-to-connect-electronic-boards-to-the-outside-world) by Nicolas to learn more about how this will work!
...
So please take a look at our alpha version of Luos web app, stay tuned and make your suggestions by sending us a message on our [Discord](https://discord.gg/luos) community!  

[Discover Luos Web App](https://app.luos.io/)

[Get Started with Luos](https://docs.luos.io/get-started/get-started)

[Benjamin Christau](https://github.com/K0rdan)[](#)