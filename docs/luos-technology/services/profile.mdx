...
Now your structure exists, you can access all variables it embeds. But it won't be updated by Luos engine as described above, for this one step remains: you have to create a service and tell Luos engine that you're using a profile and you want the platform handles it for you.
...
You can notice that you don't send any Luos message to share the button state: if an application wants to access this information, the state profile will share it for you. You only have to update the button state value in your code. Supported profiles are available in this [repository](https://github.com/Luos-io/Luos/tree/main/Engine/Profiles).
