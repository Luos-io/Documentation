...
Timestamp API also allows you to tag an event instead of a data. The idea is to create a command, attach a timestamp information to it and send it to a service.
...
In the API, **Timestamp_CreateEvent** enables you to use this feature. You have to give to this function same arguments as you give to **timestamp_tag** plus an argument called **relative_date**. This last one carries information about when the event has to be processed, its unit is **nanoseconds**. If you want your event to be processed in 1 second, put 1000000000 in this argument.
