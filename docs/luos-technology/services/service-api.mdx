...
**callback** is a pointer to a callback function called by the Luos engine when a service receives messages from other services (see [messages](../message) for more details).
...
The Luos engine enables sending and receiving messages from services with a straightforward API. More information about messages can be found in the [dedicated page](../message/basic-message).

## Luos engine APIs

The Luos engine includes the following tools to integrate more capabilities and functionalities to manage your messages:
