...
**Packages** represent these blocks of code. They contain independent functionalities which will be run by the Luos engine.
...
When we are designing a projects at Luos, we always use the same way to organize our code: we put packages into a `packages` folder (sometimes called `lib`) on a node project, and every packages have their own folder allowing them to be easily copied and pasted into any other node project:
