...
PTP is the Point-To-Point connection between nodes used for topology detection by some network (specially Robus). Every node should have between 2 to 8 PTP connections representing ports. At this time, these wires are mandatory with Robus Network.
