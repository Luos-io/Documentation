...
This project in example is Luos engine dependent. Plateformio will download from its registry, the last version of the library Luos engine .The file "node_config.h" allowing user to configure Luos parameter and hardware definition to be add to the project.
...
By default the telemetry is enable and send anonymous information to luos (only for Luos engine experience improving). To disable place the flag NOTELEMETRY in plateformio.ini the same way as LUOSHAL Flag
...
For exemple see in project gate_serialcom the plateformio.ini file
