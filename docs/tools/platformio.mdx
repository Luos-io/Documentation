...
- Telemetry value :
By default, the telemetry is enabled and send anonymous information to Luos (only for improving Luos engine experience). To disable it, place the flag `NOTELEMETRY` in _platformio.ini_ the same way that the `LUOSHAL` flag. When telemetry message appearing in red means its disable.

You must add this in your plateformio.ini like this :
```AsciiDoc
...
...
Because this example is Luos engine dependent, PlatformIO will download the last version of the Luos engine library from its registry. Every Luos project needs to have a _node_config.h_ file allowing to configure Luos engine's hardware dependencies and configure the library. See the [basics concept page](/docs/luos-technology/basics/orga) for more information.

Here is the result of a good configuration of Luos engine during compilation:

<div align="center">
  <Image src="/img/telemetry.png" darkSrc="/img/telemetry.png" />
</div>

### Definition for the tools gate and pipe

Some configuration definitions used by a script before pre-compilation may be used to configure Luos tools when they are added in your project.
For more information, see the script _select_format_script.py_ in the <a href="https://github.com/Luos-io/luos_engine/tree/main/tool_services/gate" target="_blank" rel="external nofollow">gate tool <IconExternalLink width="10" /></a> and _select_hal_script.py_ in the <a href="https://github.com/Luos-io/luos_engine/tree/main/tool_services/pipe" target="_blank" rel="external nofollow">pipe tool <IconExternalLink width="10" /></a>.

| Definition |                        Description                        |                        Value                        |
| :--------: | --------------------------------------------------------: | :-------------------------------------------------: |
| GATEFORMAT | Chooses the format of data sent by the gate through a pipe. | <a href="https://github.com/Luos-io/luos_engine/tree/main/tool_services/gate" target="_blank" rel="external nofollow">Gate message format <IconExternalLink width="10" /></a>. |
