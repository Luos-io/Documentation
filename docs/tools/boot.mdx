...
:::info
If no SERIAL_PORT is passed, pyluos-bootloader will try to find an available Gate on your system by itself.
:::
...
:::caution
In the example above, `2` is the node's ID of _node 2_, not to be confused with the service _Pipe_mod_'s ID. You can't flash one service only but the entire node, so the target must always be a node,
:::
...
#### Did Luos engine already supports your target?

The Luos engine bootloader is available for the following targets:

- STM32 F0 / F4 / G4 / L4 families
- Microchip ATSAMD21J18

Projects for each of these targets can be found in the [Luos engine example folder](https://github.com/Luos-io/Luos/tree/main/Examples/Projects). You can clone this repo and use projects for your application, or use them as examples to build your own bootloader for your specific target.

:::info
Examples are available for several IDEs: L4 / F4 / G4 / F0 uses platformIO, and SAMD21 uses MPLAB.
:::

#### Luos engine does not support your target yet

If no project is already existing for your MCU, you will have to create your bootloader project.
First of all, you have to **enable the bootloader feature** in the Luos engine. To do so, you have to add the **-D BOOTLOADER_CONFIG** parameter when you invoke your compiler. Then you have to run the library in your main() function as you would do for any project:
...
> **Warning**: Luos engine will now run the bootloader application. Be careful not to initialize any package or services. Luos engine can only run the bootloader app in bootloader mode, and your package will not be executed.

Now you have to adjust your linker settings: your bootloader has to reserve a portion of the flash, and the remaining memory will be dedicated to the Luos engine applications. You can find the memory layout of the flash resumed in the following picture:
...
:::info
You can find application examples in [the Luos engine repository](https://github.com/Luos-io/Luos/tree/main/Examples/Projects).
:::
