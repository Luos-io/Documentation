...
- **memory**: Memory statisctics information.
  ...
  - **luos_stack_ratio**: Percentage of memory occupation of Luos engine tasks.
...
Luos engine allows you to declare to an entire network a critical failure on a service.
...
Inspector, exactly like the Gate, is a Luos engine [App service](../luos-technology/services#apps-guidelines), so it is able to be hosted in any MCU running Luos engine.
...
The format of the messages that are transmitted serially form the inspector using the pipe driver, follow the specified [Luos engine communication protocol](../luos-technology/message).
...
5. Services' and Nodes' Luos engine revision
6. Verbose mode
