...
In some developments, you will need to duplicate the same service multiple times. For example, if you are making a robot with multiple motors you could have the exact same board running the exact same firmware at multiple times and at different places on your device.
...
Every service needs an alias. Basically, when you create your service you will have to define your service default's alias.
...
To make an alias resilient, we will need to save the alias information in flash or EEPROM. Depending on your hardware specificities, you may have different ways to read and write into your memory. You can use this function to store some other parameters such as configuration or calibrations values for your project, have direct flash access, or use a file system.
...
Let's use a simple les service as an example. In the package inititalization, you need to create your service with the associated alias:
...
As you can see, the service saved your alias and replace the space with the character `_`. Also, be aware that a too long alias (above 15 characters) will be truncated.

:::warning
If your alias has characters different from A to z, 0 to 9, space or `_`, it will be rejected and won't be saved.
:::
...
When a service receives a `WRITE_ALIAS` command, Luos saves the alias then passes the message to the service. This means that you can get this message into the `Led_MsgHandler` callback to save this new alias somewhere (EEPROM, Flash, SD card, etc.).
...
:::info
You can find an example of resilient aliases management as a library here: TODO
:::
