...
# Part 1: Morse encoder algorythm
...
The first step of this tutorial is to create the application service that will encode words into morse signals. The final goal is to generate an output that will be the translation of an input word into morse code.
...
- An element = state ( 0 or 1) and time (short “.” or long “-”)
  - short = 1 time to 1
  - long = 3 times to 1
  - between element = 1 time to 0
  - between letter= 3 time to 0
  - between word = 7 times to 0

## 3. Encoder application operation

First of all, we will create a new Luos service. A Luos service can be a driver or an application. Here we need to create an application that will contain not only an algorithm that encode strings into morse sequences, but also the logic to handle the link among the luos drivers which we will integrate afterwards.
...
Clone or download this [project](https://github.com/Luos-io/morse_encoder) and open it with VSCode:

<div align="center">
  <Image src="/img/tutorials/morse/morse-1-5.png" darkSrc="" />
</div>

Let’s enter into the details of the application for a better understanding

## 4. Encoder main feature

### a. Detection

Our service will have two operating modes:

1. Start a detection and if it detects the presence of an input service, it will create an output of the morse words received from it.
...
All services have to be connected on the luos network to send and receive messages. For services to be connected and recognized in the network, we have to launch a **detection**.
...
Pay attention to the second **if** statement. Here we use the **END_DETECTION** message received at the end of the detection process to set **input_detected** variable.

Also, after the END_DETECTION message, we do a research into the routing table.

:::tip
The routing table is a structure that has all the information about the nodes and services that exist in a system. Each service can recognize the characteristics of the others, by filtering the data of the routing table, and find the services that it wants to communicate.
:::
...
In the first **if** statement, notice that we created a specific **CMD** to manage this data, its used to filter messages in the handler and only process this type of messages.
...
We create **Morse** sequences from letters and send them as outputs to the Luos network.
...
The argument **state** is used to send **ON/OFF** to the output service, then we create a **msg_t** variable which we will fill with needed informations.

Once the message is created we can send it with the **Luos_SendMsg** function.

## 7. Element and dictionnary

We will use this function to create morse sequences for each letter in the alphabet. In morse, each letter can be broken down in several ON / OFF states and the duration of these states is defined in the [Morse code - Wikipedia](https://en.wikipedia.org/wiki/Morse_code).
...
Use the [file dictionnary](https://github.com/Luos-io/morse_encoder/blob/main/Morse_Encoder/lib/Encoder/dictionnary.h) to encode character into morse sequences exemple:
...
When a complete word is received (/r) **Encoder_PlayWord** will use the function **Encoder_PlayLetter** to change the state of the output depending on the element (1 or 0) and wait a the right time depending on the element (”.” or “-”)
