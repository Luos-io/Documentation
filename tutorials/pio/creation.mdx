...
Beginning with a project can be painful and a long process! New environment, new tools, ..., you may want to abandon before starting.
...
In this tutorial, we will use a ST NUCLEO F072RB board for a project creation with ST board (e)and an Arduino MKRZero board for project creation with Arduino board.
...
- Folder _include_: where you put the project header files.
- Folder _src_: where you put the put project source files.
...
Luos project uses a file, placed at the root of the folder, called _node_config.h_. This file allows you to configure Luos library and match your hardware configuration with Luos default configuration.
...
_platformio.ini_ is the project configuration file. You must fill in the file with the information relative to your project.
...
2. Fill in the _main.c_ file with the following code:
...
3. The include of _stm32f0xx_hal.h_ must be adapted to the STM familly MCU you chose. In our case, ST NUCLEO STM32F072RB → F0 family.
...
<TabItem value="Arduino" label="Arduino">
When you choose to use Arduino framworks, there is no need to declare the IRQ function.
