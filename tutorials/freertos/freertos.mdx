...
:::info
This tutorial use FreeRTOS, but the main concepts can be use for any other RTOS. The configurations option may vary depending on the RTOS API you are using.
:::

:::caution
This tutorial consider the reader as a FreeRTOS user. This is not a tutorial to learn how to use FreeRTOS.
:::

In this tutorial we will see how to use Luos with an RTOS and discovers the benefits of this combination.

## 1. Setup a PlatformIO project
...
Open Visual Studio Code and open a **Led** project corresponding to your marster board in **Luos_engine/Examples/Projects/YOUR_BOARD**.
...
Now we need to import FreeRTOS source files in our project. To do that we will use the platformio registry and add a [dependancy to FreeRTOS])(https://registry.platformio.org/libraries/bojit/PlatformIO-FreeRTOS). Then at compilation time Platformio will download it and include it on our project.

To do that open the `platformio.ini` file of your project and add a FreeRTOS dependancy : 
...
To properly work this lib need to know wich MCU family you are working on (like Luos), so you have to add a compilation option to define your platform.
For example, for STM32F0 you have to add the compilation flag `-DSTM32F0`.

:::info
More details about it on [the pltaformio library readme](https://registry.platformio.org/libraries/bojit/PlatformIO-FreeRTOS).
:::

In this tutorial we will also use [CMSIS_RTOS_V2](https://arm-software.github.io/CMSIS_5/RTOS2/html/rtos_api2.html) to have a generic RTOS API. You will have to find the lib correponding to your provider and MCU. If you prefer to directly use FreeRTOS API you will have to adapt the code of this tutorial, no big deal...
...
To work with Luos FreeRTOS don't need any specific configuration except one thing. So as you make it normally, you have to configure available options in the FreeRTOSConfig.h : FreeRTOS needs to handle three interrupters handlers (PendSV, SVC and Systick handlers) so you must not redefine these three handlers (For STM32 user they are in **stm32f0xx_it.c**).
...
:::info
If you want more information about the options displayed in FreeRTOSConfig.h, please check the [dedicated page](https://www.freertos.org/a00110.html).
:::

The only exception of this standard configuration is that Luos use the systick value provided by the HAL of your ship (allowing Luos to work in a bare metal configuration). By default FreeRTOS will pre-empt this systick and manage it's own. To keep Luos working we have to provide this systick value to the HAL level
...
Now you can build your project in VSCode and load it to your board. Once the board is flashed, you can use the `pyluos-shell` command on your computer to control your led trough python.
...
**Now you can have multiple Services across multiple nodes running in parallell on your favorite RTOS and synchronized trough Luos engine.**
