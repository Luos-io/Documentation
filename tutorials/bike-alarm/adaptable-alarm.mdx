...
To check if your app is already easy to be used with other ones, you have to ask yourself two questions:
 ...
 - Is my app ready to accept another one to run a detections?
 - Is my app controllable by any other app in a standard way?

## Detection consideration
In the basic version of the alarm app, when we initialized the function we made the app run a detection.
But some other apps may performs concurrent detection.
...
To be prepared for those situations, Luos provide an event as a message you can catch on your `AlarmController_MsgHandler` to deal with services configurations.

Let's manage this event and add our auto-update configuration in it:
...
Now on the init function we can remove the configuration management:
...
To finish, we have to prevent any loop execution if our services is currently not detected or under detection:
...
You have to try to be prepared of any usage of your app by preparing messages reception that can be useful. This is the API part of your app, so try to think it clean and flexible by using Luos Object Dictionary (OD) as much as possible.

There is one thing each app should have => **The capability to be stopped by any other**.
...
Luos engine provide a standard structure and command dedicated to this, called `control_mode`.
...
Each app should have a global variable representing the running state of the app. To manage it in our alarm app, I add this global variable:
...
Then I add the possibility to receive a control mode in my AlarmController_MsgHandler:
...
Now I can use this control_mode to make my app run or not by using :
```c
...
```
For this alarm app, I use it as a condition to execute the non-blocking blink on the loop function and for the IMU data reception.
...
Nice! Our app is now clean, and ready to properly work with any other app! Let's add a new one!
