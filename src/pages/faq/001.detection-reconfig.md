...
Using the gate you can still detect the packages of the system and can retrieve the data from the driver, as well as see the application running but the applications don't.
...
When a detection occure Luos remove all auto-update automation avoiding any trouble regarding the destination movement. So the application is not able to retrieve data from the driver anymore.

If you already manage re-detection on your app, your problem should come from your Gate configuration.
By default a gate initiate an auto-update to all the services available on the network improving a lot it's performance. But if you plan to have an application also using this feature on your network you don't want the gate to make it! You can avoid it by configuring your gate in `GATE_POLLING` mode.
To do that just add `#define GATE_POLLING` on your node_config.h. The entire list of gate configuration is available on the [`gate_config.h`](https://github.com/Luos-io/Examples/blob/master/Apps/Gate/gate_config.h) file.
